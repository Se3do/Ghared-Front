...
import { createTransaction, saveDraft, fetchTransactionDetails } from "@/lib/api";
  ...
  useEffect(() => {
    ...
    if (draftId) {
      const loadDraftDetails = async () => {
        try {
          const details = await fetchTransactionDetails(draftId);
          if (details) {
            setSubject(details.details.subject || "");
            setContent(details.details.content || "");
            if (details.details.type_id) {
              setSelectedTypeId(details.details.type_id);
            }
            // Extract receiver IDs from tracking movements
            const receiverIds = details.tracking
              ?.filter((t) => t.type === "movement")
              .map((t) => {
                // Try to extract user_id from the tracking data if available
                return (t as any).user_id;
              })
              .filter(Boolean) as number[];
            if (receiverIds.length > 0) {
              setSelectedReceivers(receiverIds);
            }
            toast.success("تم تحميل المسودة بنجاح");
          }
        } catch (error) {
          // Fallback to draftsData if details API fails
          if (draftsData) {
